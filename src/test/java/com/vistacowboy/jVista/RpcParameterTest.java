...
* @version 1.0 
*/ 
public class RpcParameterTest extends TestCase
{
    public RpcParameterTest(String name)
    {
        super(name);
    }
     ...
     *
     */
    public void testCipherPad() throws Exception
    {
        try
        {
            RpcParameter p = new RpcParameter(1, "something");
            Field f = p.getClass().getDeclaredField("CIPHER_PAD");
            f.setAccessible(true);
            String[] pad = (String[])f.get(0);
            assertEquals(20, pad.length);
            for (int rownum=0; rownum<pad.length; rownum++)
            {
                String row = pad[rownum];
                assertEquals(94, row.length());
                for (int colnum=0; colnum<row.length(); colnum++)
                {
                    char c = row.charAt(colnum);
                    assertEquals(ASCII_PAD[rownum][colnum], (int)c);
                }
            }
        }
        catch (Exception e) {}
    ...
    /**
     *
     * Constructor: RpcParameter(int type, String value)
     *
     */

    public void testRpcParameter() throws Exception
    {
        RpcParameter param = new RpcParameter(RpcParameter.LITERAL, "SomeParam");
        assertEquals(RpcParameter.LITERAL, param.getType());
        assertEquals("SomeParam", param.getValue());

        param = new RpcParameter(RpcParameter.ENCRYPTED, "lgs413");
        assertEquals(RpcParameter.LITERAL, param.getType());
//        assertTrue(re.match(r"[\x20-\x7E]{8}", param.getValue());

        param = new RpcParameter(RpcParameter.ENCRYPTED, "lgs413", 0, 1);
        assertEquals(RpcParameter.LITERAL, param.getType());
        assertEquals(" CTgF#b!", param.getValue());

        param = new RpcParameter(RpcParameter.ENCRYPTED, "lgs413", 1, 19);
        assertEquals(RpcParameter.LITERAL, param.getType());
        assertEquals("!S;|\\M&3", param.getValue());

        param = new RpcParameter(RpcParameter.ENCRYPTED, "lgs413", 12, 17);
        assertEquals(RpcParameter.LITERAL, param.getType());
        assertEquals(",P2GzBT1", param.getValue());

        LinkedHashMap<String, String> params = new LinkedHashMap<String, String>();
        param = new RpcParameter(RpcParameter.LIST, params);
        assertEquals(RpcParameter.LIST, param.getType());
        assertEquals(params, param.getParamList());
    }

    /**
     *
     * Method: isValidType(int type)
     *
     */
    public void testParamList2String() throws Exception
    {
        LinkedHashMap<String, String> param_list = new LinkedHashMap<String, String>();
        param_list.put("\"FILE\"", "200");
        param_list.put("\"FIELDS\"", "@;.01;2;4;5;.141;8;9;11;29");
        param_list.put("\"FLAGS\"", "IP");
        param_list.put("\"MAX\"", "1");
        param_list.put("\"FROM\"", "545");
        param_list.put("\"XREF\"", "#");
        RpcParameter param = new RpcParameter(RpcParameter.LIST, param_list);

        String expected = "006\"FILE\"003200t008\"FIELDS\"026@;.01;2;4;5;.141;8;9;11;29t007\"FLAGS\"002IPt005\"MAX\"0011t006\"FROM\"003545t006\"XREF\"001#f";
        String actual = param.paramList2String();
    ...
    */
    public void testIsValidType() throws Exception
    {
        ...
        /*
        try {
           Method method = RpcParameter.getClass().getMethod("isValidType", int.class);
    ...
    private static final int[][] ASCII_PAD = {
    ...
    {
        return new TestSuite(RpcParameterTest.class);
    }
} 
